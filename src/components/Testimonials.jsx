...
const Testimonials = () => {
  return (
    <motion.div 
      ...
      className='container mx-auto py-16 lg:px-32 w-full overflow-hidden' id='testimonials'>
      ...
      {/* <div className='flex flex-wrap justify-center gap-8'>
       {testimonialsData.map((testimonial, index) =>(
        <div key={index} className='max-w-[340px] border shadow-lg rouded px-8 py-12 text-center'>
     ...
     <div className='flex justify-between gap-8 w-full overflow-x-auto'>
        {testimonialsData.map((testimonial, index) => (
       <div key={index} className='w-[30%] border shadow-lg rounded px-8 py-12 text-center'>
          <img className='w-20 h-20 rounded-full mx-auto mb-4' src={testimonial.image} alt={testimonial.alt} />
          <h2 className='text-xl text-gray-700 font-medium'>{testimonial.name}</h2>
          <p className='text-gray-500 mb-4 text-sm'>{testimonial.title}</p>
         <div className='flex justify-center gap-1 text-red-500 mb-4'>
